...
SUMMARY_METRICS_PATH = OUTPUT_DIR / "etf_topn_rotation_vectorbt_metrics.csv"
REPORT_PATH = OUTPUT_DIR / "etf_topn_rotation_vectorbt_report.md"


def output_paths(top_n: int) -> dict[str, Path]:
    prefix = f"etf_top{top_n}_equal_weight_rotation_vectorbt"
    return {
...
def build_decisions_and_weights(
    ...
    for signal_date in signal_dates:
        ...
        if not positive_sector_mom.empty:
            ...
            reason = f"行业/主题 ETF 中选择动量为正的 Top{top_n}，入选标的等权持有"
            ...
            reason = f"Top{top_n} 行业/主题 ETF 无正动量，切到正动量防守资产"
...
def build_outputs(
    ...
    metrics = pd.DataFrame(
        [
            {
                "name": f"vectorbt_top{top_n}_equal_weight_rotation",
                "top_n": top_n,
                ...
                "single_asset_target_days": int(target_weights.gt(0).sum(axis=1).eq(1).sum()),
                "multi_asset_target_days": int(target_weights.gt(0).sum(axis=1).gt(1).sum()),
            },
            {
...
def write_report(summary_metrics: pd.DataFrame, all_decisions: dict[int, pd.DataFrame], all_orders: dict[int, pd.DataFrame]) -> None:
    lines = [
        ...
        "## 输出文件",
        "",
        f"- 汇总指标：`{SUMMARY_METRICS_PATH.as_posix()}`",
    ]

    for top_n in TOP_N_LIST:
        paths = output_paths(top_n)
        latest = all_decisions[top_n].iloc[-1]
        strategy = summary_metrics.loc[summary_metrics["name"].eq(f"vectorbt_top{top_n}_equal_weight_rotation")].iloc[0]
        lines.extend(
            [
                "",
                f"## Top{top_n} 最新信号与文件",
...
def main() -> None:
    ...
    write_report(summary_metrics, all_decisions, all_orders)

    print(f"vectorbt version: {vbt.__version__}")
    for top_n in TOP_N_LIST:
        paths = output_paths(top_n)
        print(f"Saved Top{top_n} daily value: {paths['daily']}")
        print(f"Saved Top{top_n} target weights: {paths['weights']}")
        print(f"Saved Top{top_n} decisions: {paths['decisions']}")
        print(f"Saved Top{top_n} orders: {paths['orders']}")
        print(f"Saved Top{top_n} metrics: {paths['metrics']}")
