...
【v2 相对 v1 的两处改动 —— 都是拿A股2002至今真实历史回测定的，不是拍脑袋】
  1. 价格双腿降权：位置×动量 历史相关性 ρ=0.78(踩过0.7红线)，本是同一个
     "价格趋势"因子的两个马甲。v1 各给2.0、合计4.0，占系统一半话语权，
     等于"价格一家独大"。v2 各降到1.25、合计2.5，把话语权让给正交的
     波动率(1.5)和量能，让五个维度真正各司其职。
  2. 加5日确认门槛：新档位连续5个交易日成立才翻脸。v1 年切换22.9次
     (每月近2回),whipsaw磨钱;v2 砍到5.6次(降77%),接上"两周确认"纪律。

【回测结论(20%波段牛熊标尺, 2002-01 ~ 2026-07, 5942个交易日)】
  改版+5日确认：方向准确率88%  年切换5.6次
  —— 用可忽略的准确度代价，换来切换次数暴降77%，最合"控回撤/求稳"的目标。
  注意：仪表盘是"当下市况识别器"，不是"未来收益预测器"。它答的是
       "现在处在牛/熊/震荡的哪一段"(准确率高)，不是"未来涨不涨"(那是噪声)。
...
用法：
  python market_regime_scored_v2.py            # 读最新市况 + 更新历史标签CSV + 画图
  python market_regime_scored_v2.py --no-net   # 只用本地缓存，不联网(离线复盘)
  python market_regime_scored_v2.py --date 2015-06-15   # 复盘指定日期市况
"""
import os
...
CFG = {
    ...
    # 【v2改动①】指标权重：价格双腿(位置/动量)ρ=0.78高度相关，各降到1.25
    #            避免"价格一家独大"，把话语权让给正交的波动率与量能。
    'w_position':       1.25,
    'w_momentum':       1.25,
    'w_amount':         1.0,
    'w_margin':         1.0,
    'w_volatility':     1.5,
    # 总分→标签：满分从7.5降到6.0，阈值等比缩到±1.5(原±2.0对应新体系)
    'attack_th':        1.5,    # 总分>=+1.5 → 进攻
    'defense_th':      -1.5,    # 总分<=-1.5 → 防御
    # 【v2改动②】确认门槛：新档位需连续N个交易日成立才翻脸(治whipsaw)
    'confirm_days':     5,
}
...
def compute_history(df):
    """对全历史逐行打分，返回带 score/label/label_confirmed 的 DataFrame。"""
    rows = []
    ...
    # 应用确认门槛，得到最终标签
    out['label'] = apply_confirm(out['label_raw'].tolist(), CFG['confirm_days'])
    return out
...
def print_latest(df, breadth, target_date=None):
    ...
    print('=' * 44)
    print(f'  市况仪表盘 v2  截至 {r["date"].date()}')
    print('=' * 44)
    ...
    print(f'>>> 市况标签  : 【{label}】  (经{CFG["confirm_days"]}日确认门槛)')
    print('=' * 44)
...
def save_and_plot(df):
    csv_path = os.path.join(OUT_DIR, 'market_regime_history_v2.csv')
    keep = ['date', 'close', 'ma', 'mom', 'amt_qtl', 'margin_chg', 'rv_qtl',
            'sc_position', 'sc_momentum', 'sc_amount', 'sc_margin', 'sc_volatility',
            'score', 'label_raw', 'label']
    ...
    try:
        ...
        ax.set_title('市况仪表盘 v2：沪深300 + 年线 + 市况标签(绿=进攻 红=防御)  [方向准确88%/年切换5.6次]')
        ax.legend(loc='upper left')
        ax.grid(alpha=0.3)
        png_path = os.path.join(OUT_DIR, 'market_regime_v2.png')
        fig.tight_layout()
